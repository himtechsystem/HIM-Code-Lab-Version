...
function packageTask(type: string, platform: string, arch: string, sourceFolderName: string, destinationFolderName: string) {
	...
	return () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(jsonEditor({ commit, date: readISODate(sourceFolderName), version, serverDownloadUrlTemplate: 'https://github.com/HIMCode/him-code/releases/download/1.112.02122/him code-reh-${os}-${arch}-1.112.02122.tar.gz' }))
			.pipe(es.through(function (file) {
