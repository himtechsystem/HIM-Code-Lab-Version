...
import { HimWhisperChannelName, IHimWhisperService } from '../../platform/himWhisper/common/himWhisper.js';
import { HimWhisperMainService } from '../../platform/himWhisper/electron-main/himWhisperMainService.js';
...
export class CodeApplication extends Disposable {
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting HIM Code');
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// HIM: local whisper.cpp STT (desktop)
		services.set(IHimWhisperService, new SyncDescriptor(HimWhisperMainService));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		const himWhisperChannel = ProxyChannel.fromService(accessor.get(IHimWhisperService), disposables);
		mainProcessElectronServer.registerChannel(HimWhisperChannelName, himWhisperChannel);
