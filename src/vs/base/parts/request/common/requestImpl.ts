...
import { bufferToStream, newWriteableBufferStream, VSBuffer } from '../../../common/buffer.js';
...
export async function request(options: IRequestOptions, token: CancellationToken, isOnline?: () => boolean): Promise<IRequestContext> {
	...
	try {
		...
		// Stream the body instead of buffering with arrayBuffer(). Some responses (e.g. SSE)
		// fail or hang when fully buffered; streaming matches server semantics and fixes
		// "Failed to fetch" / stream read errors in edge cases.
		let stream;
		if (res.body) {
			const writeable = newWriteableBufferStream();
			const body = res.body;
			void (async () => {
				...
				try {
					while (true) {
						...
						if (value && value.byteLength) {
							writeable.write(VSBuffer.wrap(value));
						}
					}
				} catch (err) {
					writeable.error(err instanceof Error ? err : new Error(String(err)));
				}
			})();
			stream = writeable;
		} else {
